...
## 2026-05-15 - Mobile Menu Keyboard Accessibility
**Learning:** Mobile menus often lack 'Escape' key support, trapping keyboard users. Focus must be explicitly returned to the trigger element when closing via keyboard to maintain context.
**Action:** Added an 'Escape' key event listener that closes the menu and calls `.focus()` on the toggle button.
