...
## 2024-05-18 - Navigation Accessibility
**Learning:** Custom mobile menus need keyboard accessibility (Escape key to close, returning focus) and scrollspy navigation needs dynamic `aria-current="page"` updates so assistive technologies reflect the current position.
**Action:** Always verify scroll events update ARIA state and ensure menus have proper keyboard handlers, avoiding only visual toggles.
