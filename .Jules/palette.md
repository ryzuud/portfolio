...
## 2024-11-21 - Dynamic Scrollspy and aria-current
**Learning:** Found that scrollspy navigation dynamically highlights the current section visually, but fails to communicate this state to screen reader users because `aria-current="page"` isn't applied to the active link. This creates a disparity between the visual experience and the accessible experience.
**Action:** Always ensure scrollspy scripts update the `aria-current="page"` attribute dynamically alongside visual CSS classes (like `.active`) to give assistive technology accurate context.
