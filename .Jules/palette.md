...
## 2024-05-22 - Mobile Menu Keyboard Accessibility
**Learning:** Custom mobile menus often trap keyboard users if they cannot easily close them without tabbing through all links or finding the close button.
**Action:** Always implement an event listener for the 'Escape' key on custom mobile menus and ensure focus correctly returns to the toggle button upon closing to maintain the user's place in the document flow.
