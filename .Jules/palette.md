...
## 2024-05-18 - Keyboard Accessible Custom Mobile Menus
**Learning:** Custom toggle buttons for mobile menus often lack full keyboard accessibility if they only listen for 'click' events. When using a keyboard, users expect to be able to close open modals or menus with the 'Escape' key, and their focus should be safely returned to the toggle button instead of being lost.
**Action:** Always implement an 'Escape' key listener to close custom navigation menus and explicitly call `.focus()` on the toggle button upon closing to maintain keyboard navigation flow.
